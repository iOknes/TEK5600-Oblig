...
def field_line(ux, uy, p0, l, dx=0.2, integrator=forward_euler, backwards=False):
    """
    ## Parameters:
    ...
    while length < l:
        if not f_stall:
            ...
            print(f"length ({length}) += {p_next - line_f[-1]} fstall: {f_stall}")
        ...
        if backwards and not b_stall:
            ...
            print(f"length ({length}) += {pb_next - line_b[-1]} bstall: {b_stall}")
...
def main():
    ...
    n = 5
    ax[2].quiver(x[::n,::n], y[::n,::n], ux[::n,::n], uy[::n,::n], scale=1000)
    #p0 = seed_uniform(0, 499, 0, 499, 20)
    p0 = seed_uniform(0, 126, 0, 126, 20)
    #p0 = seed_random(0, 499, 0, 499, 400)
    fl = [field_line(ux, uy, i, 20, integrator=forward_euler, backwards=False) for i in p0]
    flb = [field_line(ux, uy, i, 20, integrator=forward_euler, backwards=True) for i in p0]
    #fl = [field_line(ux, uy, i, 1000, integrator=runge_kutta_4, backwards=True) for i in p0]
    for i, j in zip(fl, flb):
        ax[0].plot(i[:,0], i[:,1])
        ax[1].plot(j[:,0], j[:,1])
    plt.show()
